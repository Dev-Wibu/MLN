...
const ExpandableCard: React.FC<ExpandableCardProps> = ({ title, icon, className, headerClassName, defaultExpanded = false, children }) => {
  ...
  return (
    <Card className={cn("border-blue-200 overflow-hidden transition-all duration-300", className)}>
      <CardHeader
        onClick={toggleExpand}
        className={cn("bg-gradient-to-r from-blue-100 to-sky-100 cursor-pointer flex flex-row items-center justify-between", headerClassName)}
      >
        ...
        {expanded ? (
          <ChevronUp className="h-5 w-5 text-blue-700 transition-transform duration-300" />
        ) : (
